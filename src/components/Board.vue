<template>
    <div>
      <div class="boards">
        <div v-for="(project, index) in projects" :key="index" class="board" :class="{ 'archived': project.archived }">
            ...
            <div v-for="(category, catIndex) in project.categories" :key="catIndex" class="category flex flex-col">
              <span class="flex flex-col">
                ...
                <input v-model="category.name"
                  class="list-name float-start text-center uppercase bg-slate-400 rounded-lg hover:bg-slate-800 hover:text-white p-2 font-medium"
                  @blur="updateListName(index, catIndex)" />
                <!-- Button to edit list name -->
              </span>
  
              <!-- Drag and drop area for tasks -->
                ...
                <template #item="{ element }">
                  <li class="task flex justify-between">
                    ...
                    <button @click="deleteTask(index, catIndex, element)" class="text-end">
                      <img src="../assets/delete-icon-1864x2048-bp2i0gor.png" class="h-auto w-auto max-h-4 max-w-full">
                    </button>
                  </li>
                </template>
              </draggable>
          ...
          <div class="text-center">
            <button @click="createList(index)" class="ml-2 bg-yellow-300 hover:text-white px-2 py-1 rounded-lg font-mono hover:bg-yellow-600">
              Add List
            </button>
          </div>
        </div>
      ...
      <div v-if="showModal" class="modal-overlay">
        <div class="modal fixed w-full h-full top-0 left-0 flex items-center justify-center">
          <div class="modal-container bg-white w-1/3 rounded-lg p-4">
            <div class="text-end">
              <button class="bg-red-400 rounded-lg hover:bg-red-800 px-2 py-1 mb-1 text-white" @click="closeModal">
                <img src="../assets/images.png" class="h-auto w-auto max-h-4 max-w-full">
              </button>
            </div>
            <input v-model="taskName" @keyup.enter="addTask" class="rounded-md p-2 border border-gray-300 mb-2 w-full"
              placeholder="Enter task name" />
            <div class="text-center">
              <button @click="addTask" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2  rounded">
                Add Task
              </button>
            </div>
          </div>
      ...
      <div class="text-center mt-4">
        <button class="rounded-lg px-2 py-1 text-gray-100 hover:text-white bg-blue-500 hover:bg-blue-800 font-mono" @click="createBoard">
          Create Board
        </button>
      </div>
    </div>
  ...
  const createList = (projectIndex) => {
    const newListName = prompt("Enter the name of the new list:");
    if (newListName) {
      projects.value[projectIndex].categories.push({
        name: newListName,
        color: "bg-blue-500",
        tasks: []
      });
